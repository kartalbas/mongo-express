...
function drawSparkline(canvasId, data, { color = '#3b82f6', fillAlpha = 0.15 } = {}) {
  ...
  for (const [i, datum] of data.entries()) {
    ctx.lineTo(i * stepX, h - (datum / max) * h * 0.9);
  }
  ...
  const lastY = h - (data.at(-1) / max) * h * 0.9;
  ctx.beginPath();
...
function createMonitoringComponent() {
  return {
    ...
    // F10: Kill Operation
    async killOperation(opid) {
      const ok = await ME.confirm(`Kill operation ${opid}?`);
      if (!ok) return;
      this.killingOp = opid;
      const baseHref = (globalThis.ME_SETTINGS && globalThis.ME_SETTINGS.baseHref) || '/';
      const csrfEl = document.querySelector('#monitoring-csrf');
      const headers = { 'Content-Type': 'application/json' };
      if (csrfEl) headers['X-CSRF-TOKEN'] = csrfEl.value;
      try {
        const resp = await fetch(`${baseHref}monitoring/killOp`, {
          method: 'POST',
          headers,
        ...
        if (data.success) {
          ME.toast(`Operation ${opid} killed`, 'success');
          this.currentOps = this.currentOps.filter((o) => o.opid !== opid);
          if (this.selectedOp?.opid === opid) this.selectedOp = null;
        } else {
          ME.toast(data.error || 'Failed to kill operation', 'error');
        }
      } catch (error) {
        ME.toast(error.message || 'Failed to kill operation', 'error');
      }
      this.killingOp = null;
    },
    ...
    async setProfiler() {
      ...
      try {
        ...
        console.error('Set profiler error:', error);
      }
    },
    ...
    async fetchMetrics() {
      ...
      try {
        ...
        const errEl = document.querySelector('#monitoring-error');
        if (errEl) errEl.style.display = 'none';
      } catch (error) {
        this.connected = false;
        const errEl = document.querySelector('#monitoring-error');
        if (errEl) {
          errEl.textContent = 'Monitoring error: ' + error.message;
