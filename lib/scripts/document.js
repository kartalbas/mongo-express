...
const cloneModal = document.querySelector('#cloneDocumentModal');
if (cloneModal) {
  let cloneDoc = null;
  cloneModal.addEventListener('shown.bs.modal', () => {
    const textarea = document.querySelector('#clone-document');
    if (!cloneDoc) {
      cloneDoc = editor(textarea, { readOnly: false });
    } else {
      cloneDoc.dispatch({
        changes: { from: 0, to: cloneDoc.state.doc.length, insert: textarea.value },
      });
    }
  });
}
