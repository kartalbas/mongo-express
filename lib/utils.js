...
export const colsToGrid = function (cols) {
  ...
  for (const key in cols) {
    rets[key] = cols[key].constructor.name === 'Array'
      ? cols[key].filter((col) => col.toString().slice(-6) === '.files'
          && [col.toString().slice(0, -6) + '.chunks'].filter((value) => cols[key].includes(value)))
