...
// Maximum file size for imports (16 MB)
const MAX_IMPORT_FILE_SIZE = 16 * 1024 * 1024;
...
const converters = {
  ...
  // If type == R, convert to RegExp (escape metacharacters to prevent ReDoS)
  R(value) {
    return new RegExp(value.replaceAll(/[$()*+.?[\\\]^{|}]/g, String.raw`\$&`), 'i');
  },
  U(value) {
...
// Detect type from a CSV cell value
function detectCsvType(value) {
  if (value === '' || value === null || value === undefined) return 'string';
  if (value === 'true' || value === 'false') return 'boolean';
  if (/^-?\d+$/.test(value) && !Number.isNaN(Number(value))) return 'number';
  if (/^-?\d+\.\d+$/.test(value) && !Number.isNaN(Number.parseFloat(value))) return 'number';
  if (/^[\da-f]{24}$/i.test(value)) return 'objectid';
  if (!Number.isNaN(Date.parse(value)) && /\d{4}(?:[/-]\d{2}){2}/.test(value)) return 'date';
  return 'string';
}

function convertCsvValue(value, type) {
  ...
  switch (type) {
    ...
    case 'objectid': {
      try { return ObjectId.createFromHexString(value); } catch { return value; }
    }
...
const routes = function (config) {
  ...
  const ALLOWED_LIMITS = new Set([10, 30, 50, 100, 500, 1000]);
  ...
  exp.compactCollection = async function (req, res) {
    await req.db.command({ compact: req.collectionName }).then(() => {
      ...
      req.session.error = 'Failed to compact collection.';
      console.error(error);
    });
    res.redirect(req.get('Referrer') || '/');
  };
  ...
  exp.exportCollection = async function (req, res) {
    try {
      ...
      req.session.error = 'Failed to export collection.';
      console.error(error);
  ...
  exp.exportColArray = async function (req, res) {
    try {
      ...
      req.session.error = 'Failed to export collection as array.';
      console.error(error);
      return res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.exportCsv = async function (req, res) {
    try {
      ...
      req.session.error = 'Failed to export collection as CSV.';
      console.error(error);
      return res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.reIndex = async function (req, res) {
    if (typeof req.collection.reIndex === 'function') {
      await req.collection.reIndex().then(() => {
        ...
        req.session.error = 'Failed to regenerate index.';
        console.error(error);
      });
    } else {
  ...
  exp.addIndex = async function (req, res) {
    ...
    await req.collection.createIndex(docBSON, options).then(() => {
      ...
      req.session.error = 'Failed to create index.';
      console.error(error);
      res.redirect(req.get('Referrer') || '/');
    });
  };
  ...
  exp.addCollection = async function (req, res) {
    ...
    // Build collection options for capped and time series collections
    const options = {};

    if (req.body.capped === 'true') {
      options.capped = true;
      const cappedSize = Number.parseInt(req.body.cappedSize, 10);
      if (!cappedSize || cappedSize < 1) {
        req.session.error = 'Capped collection requires a max size in bytes.';
        return res.redirect(req.get('Referrer') || '/');
      }
      options.size = cappedSize;
      const cappedMax = Number.parseInt(req.body.cappedMax, 10);
      if (cappedMax > 0) {
        options.max = cappedMax;
      }
    }

    if (req.body.timeseries === 'true') {
      const timeField = req.body.tsTimeField?.trim();
      if (!timeField) {
        req.session.error = 'Time series collection requires a time field.';
        return res.redirect(req.get('Referrer') || '/');
      }
      options.timeseries = { timeField };
      const metaField = req.body.tsMetaField?.trim();
      if (metaField) {
        options.timeseries.metaField = metaField;
      }
      if (req.body.tsGranularity) {
        options.timeseries.granularity = req.body.tsGranularity;
      }
      const expireAfter = Number.parseInt(req.body.tsExpireAfterSeconds, 10);
      if (expireAfter > 0) {
        options.expireAfterSeconds = expireAfter;
      }
    }

    await req.db.createCollection(name, options).then(async () => {
  ...
  exp.renameCollection = async function (req, res) {
    ...
    try {
      ...
      req.session.error = 'Failed to rename collection.';
      console.error(error);
      res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.updateCollections = async function (req, res) {
    await req.updateCollections(req.dbConnection).then(() => {
      ...
      req.session.error = 'Failed to update collections.';
      console.error(error);
      res.redirect(req.get('Referrer') || '/');
    });
  };
  ...
  exp.dropIndex = async function (req, res) {
    ...
    await req.collection.dropIndex(req.query.name).then(() => {
      ...
      req.session.error = 'Failed to delete index.';
      console.error(error);
    });
  ...
  // F8: Hide/Unhide Index
  exp.hideIndex = async function (req, res) {
    if (!req.query.name) {
      req.session.error = 'Error: missing name parameter';
      return res.redirect(req.get('Referrer') || '/');
    }
    ...
    try {
      await req.db.command({
        ...
        index: { name: req.query.name, hidden },
      });
      req.session.success = `Index "${req.query.name}" ${hidden ? 'hidden' : 'unhidden'}!`;
    } catch (error) {
      req.session.error = `Failed to ${hidden ? 'hide' : 'unhide'} index.`;
      console.error(error);
    }
    res.redirect(req.get('Referrer') || '/');
  };
  ...
  exp.viewAggregate = async function (req, res) {
    try {
      ...
      req.session.error = 'Failed to load aggregation view.';
      console.error(error);
      res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.exportAggregateResults = async function (req, res) {
    try {
      const { pipeline, format } = req.body;
      if (!Array.isArray(pipeline)) {
        return res.status(400).json({ error: 'Pipeline must be an array' });
      }

      // Block $out and $merge stages for safety
      for (const stage of pipeline) {
        const ops = Object.keys(stage);
        if (ops.includes('$out') || ops.includes('$merge')) {
          return res.status(400).json({ error: '$out and $merge stages are not allowed in exports' });
        }
      }
      ...
      const parsedPipeline = pipeline.map((stage) => {
        ...
        for (const [op, val] of Object.entries(stage)) {
          if (typeof val === 'string') {
            try { result[op] = JSON.parse(val); } catch { result[op] = val; }
          } else {
            result[op] = val;
          }
        }
      ...
      if (format === 'csv') {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${encodeURI(req.collectionName)}-aggregate.csv"`);
        res.setHeader('Content-Type', 'text/csv');
        res.write(csv(results));
        return res.end();
      }

      // Default: JSON
      res.setHeader('Content-Disposition', `attachment; filename="${encodeURI(req.collectionName)}-aggregate.json"`);
      res.setHeader('Content-Type', 'application/json');
  ...
  // F12: Create View from Aggregation Pipeline
  exp.createView = async function (req, res) {
    if (config.options.readOnly === true) {
      return res.status(403).json({ error: 'Read-only mode' });
    }
    try {
      const { viewName, pipeline } = req.body;
      if (!viewName || !Array.isArray(pipeline)) {
        return res.status(400).json({ error: 'viewName and pipeline are required' });
      }

      const validation = utils.validateCollectionName(viewName);
      if (validation.error) {
      ...
      await req.db.createCollection(viewName, {
        viewOn: req.collectionName,
  ...
  exp.viewValidation = async function (req, res) {
    try {
      ...
      req.session.error = 'Failed to load validation rules.';
      console.error(error);
      res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.updateValidation = async function (req, res) {
    ...
    try {
      ...
      req.session.error = 'Failed to update validation rules.';
      console.error(error);
      res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.bulkDelete = async function (req, res) {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: 'Bulk delete failed. Check server logs.' });
    }
  };
  ...
  // Schema analysis defaults
  const SCHEMA_DEFAULT_SAMPLE_SIZE = 500;
  const SCHEMA_MAX_SAMPLE_SIZE = 5000;
  const SCHEMA_SAMPLES_PER_FIELD = 5;

  exp.schemaAnalysis = async function (req, res) {
    try {
      const startTime = Date.now();
      let requestedSample = Number.parseInt(req.query.sampleSize, 10);
      if (!requestedSample || requestedSample < 1) requestedSample = SCHEMA_DEFAULT_SAMPLE_SIZE;
      if (requestedSample > SCHEMA_MAX_SAMPLE_SIZE) requestedSample = SCHEMA_MAX_SAMPLE_SIZE;

      // Get total count for confidence calculation
      const totalDocs = await req.collection.estimatedDocumentCount();
      const docs = await req.collection.aggregate([{ $sample: { size: requestedSample } }]).toArray();
      if (docs.length === 0) {
      ...
      const walkDoc = (obj, prefix) => {
        for (const key of Object.keys(obj)) {
          ...
          if (fieldMap[fullKey].samples.length < SCHEMA_SAMPLES_PER_FIELD) {
            const sample = typeof val === 'object' ? JSON.stringify(val).slice(0, 80) : String(val).slice(0, 80);
      ...
      // Confidence metrics
      const sampleRatio = totalDocs > 0 ? Math.min(docs.length / totalDocs, 1) : 1;
      let confidence = 'low';
      if (sampleRatio >= 0.5 || docs.length >= 1000) confidence = 'high';
      else if (sampleRatio >= 0.1 || docs.length >= 200) confidence = 'medium';

      const elapsedMs = Date.now() - startTime;

      res.render('partials/schema', {
        fields,
        sampleSize: docs.length,
        totalDocs,
        sampleRatio: Math.round(sampleRatio * 100),
        confidence,
        elapsedMs,
      });
    } catch (error) {
  ...
  exp.importCollection = async function (req, res) {
    ...
    const totalSize = files.reduce((sum, file) => sum + (file.size ?? file.data.length), 0);
    if (totalSize > MAX_IMPORT_FILE_SIZE) {
      return res.status(413).send(`Import too large (max ${MAX_IMPORT_FILE_SIZE / 1024 / 1024} MB)`);
    }
    ...
    try {
      ...
      res.status(500).send('Import failed. Check server logs for details.');
    }
  };

  // F2: CSV Import with Type Detection - module-level preview store
  const csvPreviews = new Map();
  let csvPreviewCounter = 0;

  exp.csvPreview = async function (req, res) {
    try {
      if (!req.files) return res.status(400).json({ error: 'Missing file' });
      const file = Object.values(req.files)[0];
      if (!file?.data) return res.status(400).json({ error: 'Bad file' });
      ...
      // Detect types from first 20 rows
      const sampleRows = records.slice(0, 20);
      const typeMap = {};
      for (const header of headers) {
        const types = sampleRows.map((row) => detectCsvType(row[header]));
        // Use most common non-string type, or string as default
        const counts = {};
        for (const t of types) { counts[t] = (counts[t] || 0) + 1; }
        let best = 'string';
        let bestCount = 0;
        for (const [t, c] of Object.entries(counts)) {
          if (t !== 'string' && c > bestCount) { best = t; bestCount = c; }
        }
        // Only use detected type if majority of samples match
        typeMap[header] = bestCount > sampleRows.length / 2 ? best : 'string';
      }

      // Store preview data with TTL
      const previewId = String(++csvPreviewCounter);
      csvPreviews.set(previewId, { records, headers, typeMap });
      // TTL: auto-clean after 5 minutes
      setTimeout(() => csvPreviews.delete(previewId), 5 * 60 * 1000);
  ...
  exp.csvImportConfirmed = async function (req, res) {
    try {
      const { previewId, typeOverrides } = req.body;
      const preview = csvPreviews.get(previewId);
      if (!preview) return res.status(400).json({ error: 'Preview expired. Please re-upload.' });

      const { records, headers } = preview;
      const typeMap = { ...preview.typeMap, ...typeOverrides };

      const docs = records.map((row) => {
        const doc = {};
        for (const header of headers) {
          doc[header] = convertCsvValue(row[header], typeMap[header]);
        }
        return doc;
      });

      const stats = await req.collection.insertMany(docs);
      csvPreviews.delete(previewId);
      res.json({ success: true, insertedCount: stats.insertedCount });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: error.message });
    }
