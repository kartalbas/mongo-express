const routes = function (config) {
  ...
  // F10: Kill Operation
  exp.killOp = async function (req, res) {
    try {
      if (!req.adminDb) {
        return res.status(403).json({ error: 'Admin access required' });
      }
      ...
      // opid can be numeric or string (e.g. "shardName:12345")
      const parsedOpid = /^\d+$/.test(String(opid)) ? Number(opid) : opid;
      await req.adminDb.command({ killOp: 1, op: parsedOpid });
