...
const index = function (config) {
  ...
  exp.hideIndex = configuredCollectionRoutes.hideIndex;
  ...
  exp.exportAggregateResults = configuredCollectionRoutes.exportAggregateResults;

  // F12: Create View from Aggregation Pipeline
  exp.createView = configuredCollectionRoutes.createView;
  ...
  // F2: CSV Import
  exp.csvPreview = configuredCollectionRoutes.csvPreview;
  exp.csvImportConfirmed = configuredCollectionRoutes.csvImportConfirmed;
  ...
  exp.killOp = configuredMonitoringRoutes.killOp;
  ...
  exp.index = async function (req, res) {
    ...
    if (config.mongodb.admin === true) {
      try {
        const info = await req.adminDb.serverStatus();
        ctx.info = info;
        if (info && info.uptime) {
          ctx.uptimeDays = Math.floor(info.uptime / 86400);
        }
      } catch (error) {
        console.error('serverStatus failed:', error);
        ctx.info = false;
        req.session.error = 'Could not retrieve server status.';
      }
    } else {
      ctx.info = false;
    }
