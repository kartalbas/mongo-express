...
const routes = function () {
  ...
  exp.getFile = async function (req, res) {
    ...
    try {
      ...
      readStream.on('error', function (err) {
        ...
        req.session.error = 'Failed to download file.';
        res.end();
      });

      readStream.pipe(res);
    } catch (error) {
      console.error('GridFS getFile error:', error);
      req.session.error = 'Failed to retrieve file.';
      return res.redirect(req.get('Referrer') || '/');
    }
  };
  ...
  exp.deleteFile = async function (req, res) {
    ...
    try {
      ...
      req.session.error = 'Failed to delete file.';
      return res.redirect(req.get('Referrer') || '/');
    }
  };
