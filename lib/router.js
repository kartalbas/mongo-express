...
const router = async function (config) {
  ...
  const csrfOptions = {
    ...
    cookieName: '__csrf',
    cookieOptions: { sameSite: 'strict', secure: config.site.sslEnabled, signed: true },
    size: 64,
  ...
  appRouter.post('/db/:database/csvPreview/:collection', mongoMiddleware, configuredRoutes.csvPreview);
  appRouter.post('/db/:database/csvImport/:collection', mongoMiddleware, configuredRoutes.csvImportConfirmed);
  ...
  appRouter.get('/db/:database/hideIndex/:collection', mongoMiddleware, configuredRoutes.hideIndex);
  ...
  appRouter.post('/monitoring/killOp', mongoMiddleware, configuredRoutes.killOp);
  ...
  appRouter.post('/db/:database/aggregateExport/:collection', mongoMiddleware, configuredRoutes.exportAggregateResults);
  appRouter.post('/db/:database/createView/:collection', mongoMiddleware, configuredRoutes.createView);
