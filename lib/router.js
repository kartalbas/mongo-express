...
const router = async function (config) {
  ...
  // Disconnect from MongoDB and return to login
  appRouter.get('/disconnect', (req, res) => {
    if (mongo && mongo.clients) {
      for (const c of mongo.clients) {
        if (c && c.client) c.client.close().catch(() => {});
      }
    }
    mongo = null;
    ...
    if (mongo === null) {
      ...
      if (req.method === 'POST') {
        ...
        if (hasServerConfig) {
          ...
          // Credentials valid, connect with pre-configured connection string
        } else {
